...
export default class User{
    ...
    index=async():Promise<UserSchema[]>=>{
        try{
    ...
    show=async(id:string):Promise<UserSchema>=>{
        try{
            const con = await client.connect()
    ...
    create=async(u:UserSchema):Promise<UserSchema>=>{
        try{
            const con = await client.connect()
            ...
            const result = await con.query(sql,[u.userName,hash,u.firstName,u.lastName,u.telephone])
            const user = result.rows[0]
            con.release()
            return user
        }catch(err){
    ...
    update=async(id:string,u:UserSchema):Promise<UserSchema>=>{
        try{
            const con = await client.connect()
            const sql = `update  users SET password=$1 , firstName=$2 , lastName=$3 , telephone=$4 WHERE id=$5 RETRNING *`
            const result = await con.query(sql,[u.password,u.firstName,u.lastName,u.telephone,u.id])
            const user = result.rows[0]
            con.release()
            return user
        }catch(err){
    ...
    delete=async(id:string):Promise<UserSchema>=>{
        try{
            const con = await client.connect()
